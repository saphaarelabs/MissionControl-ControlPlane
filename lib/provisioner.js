...
async function reconcileNodeCapacity(node) {
    if (!node?.id || !node?.ip_address || node.ip_address === '0.0.0.0') {
        return { nodeId: node?.id || null, skipped: true, reason: 'missing_ip' };
    }

    const res = await fetch(`http://${node.ip_address}:${LOCAL_API_PORT}/api/internal/containers`, {
        method: 'GET',
        headers: {
    ...
    if (!res.ok) {
        throw new Error(`node ${node.id} containers probe failed (${res.status}): ${await res.text()}`);
    }

    const payload = await res.json().catch(() => ({}));
    const actualUsed = Number.isFinite(payload?.total) ? payload.total : 0;
    const nextStatus = actualUsed >= node.capacity_max ? 'full' : 'ready';

    const { error } = await supabase
        .from('vps_nodes')
        .update({
            capacity_used: actualUsed,
            status: nextStatus,
        })
        .eq('id', node.id);

    if (error) {
        throw new Error(`failed to persist node ${node.id} reconciliation: ${error.message}`);
    }

    console.log(`[provisioner] reconciled node ${node.id}: used=${actualUsed}/${node.capacity_max} status=${nextStatus}`);
    return {
        nodeId: node.id,
        actualUsed,
        status: nextStatus,
...
async function reconcileFleetCapacity() {
    const { data: nodes, error } = await supabase
        .from('vps_nodes')
        .select('*')
        .in('status', ['ready', 'full']);

    if (error) {
        throw new Error(`load vps_nodes for reconciliation: ${error.message}`);
    }

    const results = [];
    for (const node of nodes || []) {
        try {
            results.push(await reconcileNodeCapacity(node));
        } catch (err) {
            console.warn(`[provisioner] reconcile failed for node ${node.id}: ${err.message}`);
            results.push({
                nodeId: node.id,
                error: err.message,
            });
...
export async function provisionUser(userId, username, onboardingData = null) {
    ...
    if (existingProfile) {
        ...
        // Note: We don't check for 'provisioning' status here because:
        // 1. The provisioning lock (acquired by backend) already prevents duplicate provision attempts
        // 2. Checking status === 'provisioning' creates a race condition where the lock acquisition
        //    changes status to 'provisioning', then this check prevents actual provisioning
        // 3. If we get here, it means backend acquired the lock and expects us to provision
    }
    
    let node = await findAvailableNode();

    if (!node) {
        console.warn(`[provisioner] no immediately claimable node for ${userId} — reconciling fleet capacity before queueing a new VPS`);
        try {
            const reconciliation = await reconcileFleetCapacity();
            console.log(`[provisioner] fleet reconciliation results for ${userId}: ${JSON.stringify(reconciliation)}`);
            node = await findAvailableNode();
        } catch (reconcileErr) {
            console.warn(`[provisioner] fleet reconciliation failed for ${userId}: ${reconcileErr.message}`);
        }
    }
    ...
    try {
        ...
        // Configure LLM provider if onboarding data includes API key
        if (onboardingData?.llm?.apiKey && onboardingData?.llm?.provider) {
            console.log(`[provisioner] configuring LLM provider ${onboardingData.llm.provider} for ${userId}...`);
            try {
                const configureResponse = await fetch(`http://${node.ip_address}:${LOCAL_API_PORT}/api/internal/configure-provider`, {
                    method: 'POST',
                    headers: {
                    ...
                    body: JSON.stringify({
                        instanceId: userId,
                        provider: onboardingData.llm.provider,
                        token: onboardingData.llm.apiKey,
                        authMethod: 'api_key'
                    })
                });

                if (configureResponse.ok) {
                    console.log(`[provisioner] ✓ LLM provider ${onboardingData.llm.provider} configured for ${userId}`);
                } else {
                    const errorText = await configureResponse.text().catch(() => '');
                    console.warn(`[provisioner] ⚠️  LLM configuration failed for ${userId}: ${configureResponse.status} ${errorText.slice(0, 200)}`);
                }
            } catch (err) {
                console.warn(`[provisioner] ⚠️  LLM configuration error for ${userId}: ${err.message}`);
                // Don't fail provisioning if LLM config fails - user can configure it later via UI
            }
        }
...
OPENCLAW_MEM0_API_KEY=${process.env.OPENCLAW_MEM0_API_KEY || ''}
OPENCLAW_COMPOSIO_API_KEY=${process.env.OPENCLAW_COMPOSIO_API_KEY || ''}
OPENCLAW_DEFAULT_PROVIDER_KEY=${process.env.OPENCLAW_DEFAULT_PROVIDER_KEY || 'cloudflare-nemotron'}
OPENCLAW_DEFAULT_PROVIDER_BASE_URL=${process.env.OPENCLAW_DEFAULT_PROVIDER_BASE_URL || ''}
OPENCLAW_DEFAULT_PROVIDER_API_KEY=${process.env.OPENCLAW_DEFAULT_PROVIDER_API_KEY || ''}
OPENCLAW_DEFAULT_PROVIDER_API=${process.env.OPENCLAW_DEFAULT_PROVIDER_API || 'openai-completions'}
OPENCLAW_DEFAULT_PRIMARY_MODEL=${process.env.OPENCLAW_DEFAULT_PRIMARY_MODEL || 'cloudflare-nemotron/workers-ai/@cf/nvidia/nemotron-3-120b-a12b'}
ENVEOF
